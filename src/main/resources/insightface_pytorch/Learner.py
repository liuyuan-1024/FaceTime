...
class face_learner(object):
    def __init__(self, conf, inference=False):
        ...
        if not inference:
            ...
            self.step = 0
            self.head = Arcface(embedding_size=conf.embedding_size, classnum=self.class_num).to(
                conf.device)
    ...
    def save_state(self, conf, accuracy, to_save_folder=False, extra=None, model_only=False):
        ...
        torch.save(
            self.model.state_dict(), save_path /
                                     ('model_{}_accuracy:{}_step:{}_{}.pth'.format(get_time(),
                                                                                   accuracy,
                                                                                   self.step,
                                                                                   extra)))
        if not model_only:
            torch.save(
                self.head.state_dict(), save_path /
                                        ('head_{}_accuracy:{}_step:{}_{}.pth'.format(get_time(),
                                                                                     accuracy,
                                                                                     self.step,
                                                                                     extra)))
            torch.save(
        ...
        for e in range(epochs):
            ...
            for imgs, labels in tqdm(iter(self.loader)):
                ...
                if self.step % self.evaluate_every == 0 and self.step != 0:
                    ...
                    accuracy, best_threshold, roc_curve_tensor = self.evaluate(conf, self.lfw,
                                                                               self.lfw_issame)
