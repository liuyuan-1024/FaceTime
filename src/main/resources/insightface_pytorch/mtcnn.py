...
class MTCNN():
    ...
    def align(self, img):
        ...
        warped_face = warp_and_crop_face(np.array(img), facial5points, self.refrence,
                                         crop_size=(112, 112))
    ...
    def detect_faces(self, image, min_face_size=20.0,
        ...
        with torch.no_grad():
            ...
            landmarks[:, 0:5] = np.expand_dims(xmin, 1) + np.expand_dims(width, 1) * landmarks[:,
                                                                                     0:5]
            landmarks[:, 5:10] = np.expand_dims(ymin, 1) + np.expand_dims(height, 1) * landmarks[:,
                                                                                       5:10]
